...
let time = new Date();
!(async () => {
  if ($request.url.indexOf("iid=" && "device_id=") != -1) {
    var Cookie = $request.headers;
    /* var url = $request.url;
    var iid = url.split(/iid=(.+?)&/)[1];
    var device = url.split(/device_id=(.+?)&/)[1];
    var session = Cookie.split(/sessionid=(.+?);/)[1];
    token = await getToken(ql);
    var result = await SetEnvs(ql, token, $.name, "sessionid=" + session + "&iid=" + iid + "&device_id=" + device);
 */
    $notify("抖音极速版", $.name, Cookie);
  }
})()
